...
const ENDPOINT = 'https://api.atmo.example';
...
describe('public contract', () => {
	it('keeps profile and follow projections internal and validation disabled', () => {
		...
		expect(config).not.toHaveProperty('spaces');
	});

	it('preserves the existing atmo.rsvp event query contract', () => {
		expect(config.collections.event?.queryable).toEqual({
			mode: {},
			name: {},
			status: {},
			description: {},
		...
		expect(config.collections.event?.searchable).toEqual(['mode', 'name', 'status', 'description']);
		expect(Object.keys(config.collections.event?.pipelineQueries ?? {}).sort()).toEqual([
			'listAuthored',
			'listDiscoverable',
	...
	it('advertises the exact generated calendar API', async () => {
		...
		expect(service.manifest.serviceAuth).toEqual({
			...
			serviceDid: 'did:web:api.atmo.example',
			audience: 'did:web:api.atmo.example#contrail',
			scope:
				'rpc?aud=did:web:api.atmo.example%23contrail&lxm=rsvp.atmo.getFeed&lxm=rsvp.atmo.notifyOfUpdate',
	...
	it('publishes its service DID and permits browser auth headers', async () => {
		...
		expect(await did.json()).toMatchObject({
			id: 'did:web:api.atmo.example',
			service: [
				{
					id: 'did:web:api.atmo.example#contrail',
