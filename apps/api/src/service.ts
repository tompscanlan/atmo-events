...
/**
 * Origin used when `PUBLIC_SERVICE_ENDPOINT` is unset. `.test` is reserved by
 * RFC 6761, so this can never be a real deployment. wrangler.jsonc sets the
 * deployed origin.
 *
 * It is HTTPS even for local runs because Contrail's `createWorker` rejects a
 * plain-HTTP public endpoint, and a `did:web:localhost%3A<port>` document URL
 * resolves back to HTTP, which its audience check then refuses.
 * `wrangler dev` still serves on http://localhost:8787; only the advertised
 * discovery identity is this origin.
 */
export const DEFAULT_PUBLIC_SERVICE_ENDPOINT = 'https://api.atmo.test';
...
export function serviceAudience(endpoint: string, fragment: string): `did:web:${string}#${string}` {
