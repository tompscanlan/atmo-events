/**
 * openmeet-atmo API: one Worker, two runtimes.
 *
 * - Contrail serves the anonymous public index (`rsvp.atmo.*`), which includes
 *   public group events written into a group account's public repo.
 * - The Spaces provider serves the members-only slice (`net.openmeet.group.*`)
 *   plus the AT Protocol Space callbacks (`com.atproto.space.*`).
 *
 * Both runtimes answer `GET /.well-known/did.json` and `GET /lexicons`, so a
 * naive "try one, fall through to the other" dispatcher would silently publish
 * only one service entry and hide the other half of the deployment. Those two
 * paths are therefore merged explicitly; everything else routes by prefix.
 */
...
import { createGroupSpaces, SPACES_NAMESPACE, type OpenmeetApiEnv } from './spaces';

/** The part of Contrail's prebuilt Worker entrypoint this dispatcher drives. */
export interface ContrailRuntime {
	fetch(request: Request, env: OpenmeetApiEnv, ctx?: ExecutionContext): Promise<Response>;
	scheduled(event: ScheduledEvent, env: OpenmeetApiEnv, ctx: ExecutionContext): Promise<void>;
}

export type SpacesRuntime = SpacesWorkerHandler<OpenmeetApiEnv>;
 ...
 */
function runtimes(env: OpenmeetApiEnv): ApiRuntimes {
	const endpoint = publicServiceEndpoint(env);
	if (cached?.endpoint === endpoint) return cached;
	const contrail = createWorker<OpenmeetApiEnv>(contrailConfigFor(endpoint), {
		lexicons,
