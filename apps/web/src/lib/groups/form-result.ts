...
// wholesale on the client with generated call proxies, so a plain helper
// exported from there would not survive the trip.
//
...
// lets a page read `.error` off a success — the exact bug this file exists to
// make impossible. Every handler therefore annotates its return type as
// `GroupFormResult<…>`, and pages narrow with `groupFormError` / `result.ok`.
...
/** The message, or `undefined` when there is none — the only route from a
