...
// gets signed by the group DID. Split out of groups.remote.ts so the shape a
// form produces can be asserted without a request: the live probe
// (scripts/groups-e2e.mjs) and event-record.test.ts drive this exact function,
// not a copy of it.
//
 ...
 * (2..10 chars), so a location with no country is not an address and must not
 * be written: `writeGroupEvent` validates the whole record and would refuse it
 * entirely, taking the event with it. That was a live bug — every group event
 * carrying a location name was rejected, because this path used to send
 * `country: ''` (found by scripts/groups-e2e.mjs against the fixture PDS).
 *
 * So an entry is built only when it satisfies the lexicon, and `locations` is
 * omitted rather than left empty — the same "include only what exists" rule the
 * app's own event editor applies (packages/ui/src/editor/LocationSection.svelte
