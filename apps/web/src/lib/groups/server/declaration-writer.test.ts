// The declaration writer. Three things here are worth a test and the rest is
// not: that the record goes to the PUBLIC REPO rather than a space (it is the
// only group record that must be anonymously readable, so a stray `space` would
// silently make the group undiscoverable while every "the record was written"
// assertion still passed); that a PRIVATE group's declaration is DELETED rather
// than merely not rewritten (absence is the whole signal, so a stale pointer
// keeps announcing a group that asked not to be announced); and that the gate
// is MANAGE_GROUP, since announcing a group to the network is changing its face.
