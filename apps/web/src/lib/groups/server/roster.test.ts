...
//
// Every roster act is a D1 row move plus a `membership` record write. The gate
// resolves from records, so the order decides which way a partial failure
...
//     the old, smaller grant in the record;
//   * a revocation (leave, eject, demotion) runs a read-only pre-check, then
//     the record, then the row, so a failed row write leaves a record that
//     already grants less.
//
...
describe('a revocation writes the record first', () => {
