// The settings-page repair. What is load-bearing is what it REFUSES to write:
// it copies the row into the records only where the row is certain, so the
// cases that matter are the ones where writing would be wrong. Those are a
// non-owner row with no record (a failed grant looks the same as a failed
// removal), a record that disagrees with its row, and an authz config that is
// half there.
...
describe('repairGroup', () => {
	...
	// Alpha Shakeout's shape: a later member's record exists, the owner's does
	// not, and there is no authz config.
