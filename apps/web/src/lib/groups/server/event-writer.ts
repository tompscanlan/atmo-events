// THE WRITE GATE.
//
// Every write helper in $lib/atproto/server/repo.remote.ts sets
...
// group's admins co-edit one record set rather than each other's repos. On
// atproto that means the write must be authored by the group's own DID, because
// a repo write requires `repo === the authenticated DID`.
//
// So this path never uses `locals.did` as the write target. `locals.did` is only
// the subject of a permission check; the credential is the group's, held by the
// app (see ./credentials.ts), and `repo` is always `group.group_did`.
//
// Public group events go to the group DID's public repo, where anyone can read
// them and Contrail indexes them. A space is never anonymously readable, so
// only records for members belong in the group's spaces.
...
export interface GroupRepoWrite {
	/** Always the group DID. See the file header. */
	repo: string;
	collection: string;
	rkey: string;
	record: Record<string, unknown>;
	intent: 'create' | 'update' | 'delete';
	/** Space URI when this record belongs in one of the group's spaces
	 *  (`$lib/groups/server/spaces.ts`), absent when it belongs in the group's
	 *  public repo.
	 *
	 *  Both targets are in use. Public group events must stay plain repo
	 *  records: that is what Jetstream sees and what Contrail indexes, and a
	 *  space is never anonymously readable. The control plane lives in spaces.
	 *  So the target is chosen per write, not per writer. */
	space?: string;
}

/** The transport half of the gate: takes an already-authorised write and puts
 *  it in the group's repo. Injectable so the permission and authorship
 *  decisions can be tested without a live PDS. */
export type GroupRepoWriter = (write: GroupRepoWrite) => Promise<{ uri: string; cid: string }>;

/** The real transport: a password session for the group account, then the
 *  write. `groupDid` is checked against the session the PDS returns, so a
 *  credential stored under the wrong group cannot quietly write elsewhere.
 *
 ...
 *  whose part of the space is being written, which for every record the group
 *  writes is the group itself. */
export function pdsWriter(cred: GroupCredential, groupDid: string): GroupRepoWriter {
	return async (write) => {
		...
		// @atcute's input types want an ActorIdentifier, and this check is the
		// one place that can produce one. A group row whose DID is malformed must
		// fail here, not get past the type with a cast.
		...
		// generated lexicon set (see `groupClient`). The repo methods stay on the
		// typed client, which validates their inputs.
...
export interface GroupEventWriteResult {
	...
	cid: string;
	rkey: string;
	/** Echoed back so a caller can check what every test here checks: the
 ...
 *  event. There is no "the event's own creator may edit it" rule: the author
 *  of every group event is the group, so there is no per-event creator to
 *  compare against, and no provenance row records one. With the default
 *  bundles this costs nothing: the roles that hold CREATE_EVENT (owner, admin)
 *  also hold MANAGE_EVENTS. */
...
/** The gate's permission half, on its own so another record type does not
 *  have to re-derive it. Events reach it through `authorise` below; the control
 *  plane calls it directly (`./about-writer.ts`), because its permission is
 *  fixed rather than chosen by intent.
 *
 *  Refuses an anonymous caller before touching D1 or the PDS. What the caller
 *  may do is decided by the group's records, with the fallback and fail-closed
 *  policy that `getCallerMembership` documents. */
 ...
 *
 *  A failure is logged and swallowed, here rather than in each notifier. By the
 *  time this runs the PDS has accepted the record, so a dead index means the
 *  events tab is stale, not that the write failed. Reporting it as a failed
 *  write would be untrue and would invite the caller to retry a write that
 *  landed. */
...
export async function writeGroupEvent(input: WriteGroupEventInput): Promise<GroupEventWriteResult> {
	...
	// A second check: if the authority in the returned URI is not the group,
	// the record did not land where it must, and the caller must not be told
	// the write succeeded.
	...
	// index has to be told. It happens here rather than in each route because
	// this is the one function that writes a group event. A second write path
	// that forgot this call would ship a tab that silently stops updating.
...
export async function deleteGroupEvent(
	...
	// that did not notify would leave the event on the tab, because the
	// backfill has already completed and does not run again.
	await notifyIndex(input, result.uri);
 ...
 *
 *  The credential is the app password stored when the group was created
 *  (`resolveGroupCredential`). Without one, this throws GroupCredentialError. */
