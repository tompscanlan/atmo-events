// A D1Database over `node:sqlite`, so the groups tests run the real SQL (the
// triggers, the CHECKs, the partial unique indexes) instead of a mock that
// would agree with whatever the code did. D1 is SQLite, so this is the same
// engine that enforces the invariants in production.
//
// Test harness only. It implements just the D1 surface $lib/groups/server uses
// (prepare/bind/all/first/run and batch-as-transaction).
 ...
 *  migration test itself, which applies the statements on purpose (and twice,
 *  to prove they are idempotent). On everywhere else: `ensureGroupsSchema`
 *  memoizes per isolate, so a second in-memory database in the same test file
 *  would otherwise get no schema at all. */
export function sqliteD1(applySchema = true): SqliteD1 {
	...
	// the composite (role_id, group_id) -> roles(id, group_id) key, which is the
