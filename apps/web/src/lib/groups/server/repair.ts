...
// group's settings page, behind MANAGE_GROUP, in two steps and always in this
// order:
//
...
//      so every profile-owned column and every roster row says what the
//      records say.
//
// WHY STEP 1 EXISTS. A create writes the group's row before its records
// (`../create-group.ts`), so a PDS failure after the INSERT leaves a group
// whose roster lives only in the database. That group works while its members
// space is empty, because every reader falls back to the rows. But the next
// roster act writes ONE member's record, the gate starts resolving from
// records, and everyone else, owner included, holds nothing. A group created
// before the record writers existed has the same shape.
//
// WHY IT ONLY FILLS GAPS. The records are the source of truth, and this is the
// one place that copies the row into them, so it copies only what is missing
// and only what the row is certain of. A record that exists is never rewritten.
//
...
//   * ANY OTHER member's row without a record is NOT. It is what a grant whose
//     record write failed leaves, and also what a revocation leaves when its
//     record went but its row delete failed. Writing it would re-grant someone
//     who was removed, so it is reported, as `rebuildGroupMembers` reports it,
//     and never written. A role change on the members page writes it.
//   * The authz config is written only when the space holds NONE of it, and
//     only when no such member is waiting: once it exists the gate resolves
//     from records, and a member with no record would silently lose access. A
//     partial config (some of its three kinds present) is reported rather than
//     completed, because completing it from the row would overwrite the half
//     that exists.
//
// Idempotent: a second run finds nothing missing and writes nothing, and the
// rebuild it ends with is itself a no-op over agreeing data.
//
// WHAT IT CANNOT REACH. The gate is MANAGE_GROUP, resolved like every other
// gate. So a group whose authz config exists while the OWNER has no membership
// record refuses its own owner here too, since the records grant them nothing.
// A create cannot leave that shape, because it writes the owner's record before
// the config. Only an edit of the config itself could, which is what the
// owner-keeps-MANAGE_GROUP invariant for binding edits is meant to prevent.
