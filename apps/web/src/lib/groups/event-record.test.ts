// THE ADDRESS RULE. The address lexicon requires `country` (2 to 10
// characters). A location sent with `country: ''` fails `writeGroupEvent`'s
// validator, which then refuses the whole event with the unhelpful "that is not
// a valid community.lexicon.calendar.event record".
//
...
describe('group event locations', () => {
	...
	// The regression guard: an address entry with an empty country would make
	// this write throw GroupRecordError.
