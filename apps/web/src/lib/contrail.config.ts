...
export const config: ContrailConfig = {
	...
	collections: {
		...
		// A public group's declaration, indexed off Jetstream like any other
		// actor's record: no group credential, no space config. Present means
		// public, absent means private — the writer deletes it on the flip and
		// Jetstream carries the delete — so the list needs no visibility filter.
		// Nothing notifies the index on a declaration write; the cron picks it up.
		declaration: {
